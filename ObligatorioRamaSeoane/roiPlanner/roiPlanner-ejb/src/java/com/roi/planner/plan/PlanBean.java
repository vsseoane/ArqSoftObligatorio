...
public class PlanBean {
    ...
    public List<Plan> getPlansFromProgrammer() {
        int approved = 1;
        return em.createNativeQuery("select * from Plans as p where "
                + "p.ISAPPROVED=" + approved, Plan.class).getResultList();
    }
    ...
    public void deletePlanFromOrder(Order order) throws OrderNotFoundException {
        try {
            Plan plan = findOrder(order.getOrderNumber());
            if (plan != null) {
    ...
    public Plan findOrder(int id) throws OrderNotFoundException {
        List<Plan> plans =  em.createNativeQuery("select * from PLANS where IDORDER=" 
                + id,Plan.class).getResultList();
        return plans.get(0);
    }
    
    public Plan find(int id) throws OrderNotFoundException {
        try {
            Plan plan = (Plan) em.createNativeQuery("select * from PLANS"
                    + " where ID=" + id, Plan.class).getResultList().get(0);
            return plan;
        } catch (Exception e) {
            throw new OrderNotFoundException();
        }
    ...
    public List<Stretch> getStretches() {
        try {
            ...
            String jsonStretch = client
                    .target("https://pipeline-calculator-api."
                            + "herokuapp.com/pipeline-route/service/" + 1)
                    .request(MediaType.APPLICATION_JSON)
                    .post(Entity.json(null), String.class); 
            HashMap var = gson.fromJson(jsonStretch, HashMap.class);
            ArrayList<LinkedTreeMap> lista = (ArrayList<LinkedTreeMap>) var
                    .get("sections");
    ...
    public void secondApprovePlan(int idPlan) throws 
            PlanNotFoundException, NotExistFirstApprovedException {
        try {
            Plan plan = find(idPlan);
    ...
    public void sendCommandsToGoliath(Plan plan) {
        ...
        List<ActuatorProgramming> actuatorProgramming = em
                .createNativeQuery("select * from ActuatorProgramming", 
                        ActuatorProgramming.class).getResultList();
        ...
        restBean.postMethod("http://localhost:8080/roi-goliath-war/api/devices",
                request);             
    }
